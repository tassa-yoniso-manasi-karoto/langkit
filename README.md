...
> [!IMPORTANT]
> **Some features use machine learning models** for tasks like speech-to-text and voice separation. Langkit supports both **local processing** (free via <img src="internal/docker.svg" height="16"> or custom endpoints like llama.cpp, LocalAI...) and **cloud APIs** (Replicate, OpenAI, ElevenLabs: pay-as-you-go, typically pennies per file). <br> Custom endpoints are your responsibility to set up and run, Langkit just connects to them. Configure custom endpoints options in Settings. 
...
Langkit can run as a standalone but it can now also run directly inside Anki. This should offer better reliability and somewhat better performance so **it's the recommended way to use Langkit.**
...
Additionally a **summary of the episode/media can be generated by an AI (LLM)** in your native language and embeded in the audiofile to refresh your memories before listening and help your understand the condensed audio's content. OpenRouter serves a number of LLMs that can do this **for free** (Deepseek, Qwen...), or you can use a custom local endpoint (e.g., llama.cpp, Ollama...). 
...
This feature works by merging the original audiotrack with negative gain together with an audiotrack containing the voices only with additional gain. Obtaining the isolated voice track requires running one of these deep learning audio separation tools:

> [!NOTE]
> **Docker-based options** run locally on your machine for free. They work on CPU (very slow) or with <img src="internal/nvidia.svg" height="16"> GPU acceleration (x50-70 time faster). The Docker image will be pulled automatically on first use (~3GB download). <br> Processing time on CPU is roughly 1.5x the track duration.
...
| docker-nvidia-demucs, dn       | good    | FREE (local) | MIT license | <img src="internal/windows.svg" height="16"> works out of the box <img src="internal/linux.svg" height="20"> [see setup](#how-do-i-use-the-gpu-accelerated-voice-separation-on-linux) |
| docker-nvidia-demucs_ft, dnf   | good    | FREE (local) | MIT license | <img src="internal/windows.svg" height="16"> works out of the box <img src="internal/linux.svg" height="20"> [see setup](#how-do-i-use-the-gpu-accelerated-voice-separation-on-linux) |
| **CLOUD (REPLICATE)** | ——— | ————— | ———— | ———————————————— |
...
| replicate-spleeter, sp         | rather poor | very cheap 0.00027$/run | MIT license | Requires Replicate API token |
| **OTHER** | ——— | ————— | ———— | ———————————————— |
| elevenlabs, 11, el             | good    | very expensive 1$/*MINUTE* | proprietary | Not supported on GUI. <br><sub>mp3 only which desyncs audio. Also distorts soundstage.</sub> |
...
#### I get a blue popup that says "Windows protected your PC" error when trying to run Langkit.
</summary>
...
> -   <img src="internal/windows.svg" height="16"> **Windows:** `%APPDATA%\langkit\config.yaml` (use Notepad++ to open it)
> -   <img src="internal/linux.svg" height="20"> **Linux:** `~/.config/langkit/config.yaml`
> -   <picture> <source media="(prefers-color-scheme: dark)" srcset="internal/macos-dark.svg" height="16"> <img src="internal/macos-light.svg" height="16">  </picture> **macOS:** `~/Library/Application Support/langkit/config.yaml`
