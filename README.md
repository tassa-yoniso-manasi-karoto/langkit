...
- **Dubtitles as source of truth for subtitle lines** (when both are selected together)

# Condensed Audio
langkit can **make an audio file containing all the audio snippets of dialog** in the audio track. <br>
...
Additionally a **summary of the episode/media can be generated by an AI (LLM)** in your native language and embedded in the audiofile to refresh your memories before listening and help you understand the condensed audio's content. OpenRouter serves a number of LLMs that can do this **for free** (Deepseek, Qwen...), or you can use a custom local endpoint (e.g., llama.cpp, Ollama...). 
...
This feature works by merging the original audio track with negative gain together with an audio track containing the voices only with additional gain. Obtaining the isolated voice track requires running one of these deep learning audio separation tools:

> [!NOTE]
> **Docker-based providers** run locally on your machine for free. They work on CPU (very slow) or with <img src="internal/nvidia.svg" height="16"> GPU acceleration. Docker-based providers are downloaded & installed automatically on first use, you only need to have Docker Desktop installed.
> <br> On Windows <img src="internal/windows.svg" height="16"> GPU mode works out of the box, <img src="internal/linux.svg" height="20"> requires [NVIDIA Container Toolkit setup](#how-do-i-use-the-gpu-accelerated-voice-separation-on-linux).
...
- Generious free tier. No credit card required for free access but rate limited.
