...
> [!NOTE]
> **Docker-based options** run locally on your machine for free. They work on CPU (very slow) or with <img src="internal/nvidia.svg" height="16"> GPU acceleration (x50-70 time faster, minimum 2GB of VRAM needed). The Demucs Docker image will be pulled downloaded & installed on first use (~7GB download). <br> Processing time on CPU is roughly 1.5x the track duration.
