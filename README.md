...
> [!IMPORTANT]
> **Some features use machine learning models** for tasks like speech-to-text and voice separation. Langkit supports both **local processing** (free via Docker or custom endpoints like llama.cpp, LocalAI...) and **cloud APIs** (Replicate, OpenAI, ElevenLabs: pay-as-you-go, typically pennies per file). Custom endpoints are your responsibility to set up and run, Langkit just connects to them. Configure custom endpoints options in Settings. 
...
> [!NOTE]
> **Docker-based options** run locally on your machine for free. They work on CPU (slower) or with Nvidia GPU acceleration (faster). The Docker image will be pulled automatically on first use (~3GB download). <br> Processing time on CPU is roughly 1.5x the track duration.
