...
<div class="docker-container">
    <svg
        ...
        height={size}
        viewBox="0 0 24 24"
        class={`${className} docker-icon ${isAnimating ? 'animating' : ''}`}
        on:mouseenter={startAnimation}
...
<style>
    ...
    .docker-icon.animating {
        animation: seaRoll 2.2s ease-out;
    }
    ...
    @keyframes seaRoll {
        ...
        25% {
            transform: rotate(12deg) translateX(2px) translateY(-0.5px);
        }
        40% {
            transform: rotate(-8deg) translateX(-1.5px) translateY(0.5px);
        }
        55% {
            transform: rotate(5deg) translateX(1px) translateY(-0.25px);
        }
        70% {
            transform: rotate(-3deg) translateX(-0.5px) translateY(0.25px);
        }
        85% {
            transform: rotate(1deg) translateX(0.25px) translateY(-0.1px);
        }
        100% {
            transform: rotate(0deg) translateX(0) translateY(0);
        }
