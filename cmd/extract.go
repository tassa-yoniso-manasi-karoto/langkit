...
func init() {
	...
	extractCmd.PersistentFlags().StringSliceVarP(&langs, "langs", "l", []string{}, "ISO-639-1 codes of target language followed by reference language(s) sorted of preference (ie. learning spanish from english: \"es,en\")")
	...
	extractCmd.PersistentFlags().StringVar(&STT, "stt", "", "transcribe audio using an online Speech-To-Text API")
	...
	extractCmd.PersistentFlags().Int("offset", 250, "pad timings of each audio clip with this offset in millisecond. Useful to compensate inaccurate sync between subs and voices.")
	// uh? when using extractCmd.PersistentFlags().IntP <-- default negative int is reset to 0. maybe force signed integer?
	extractCmd.PersistentFlags().Int("a", -1, "force selection of the audiotrack at this index. Useful for audiotracks missing a language tag. Overrides --chan and -l flag. Indexing start at 1.")
